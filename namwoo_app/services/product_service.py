...
def find_batteries_for_vehicle(
    ...
    """
    Orchestrates an intelligent "Parse, Filter, then Decide" search flow:
    1. Use an AI to parse the user's query into structured data (make, model, year).
    2. Build a precise, filtered database query using this structured data.
    3. If multiple candidates are found, use a second AI call to semantically decide the best match.
    4. Return a single, confident result or a clean "not_found" status.
    """
    if not user_query:
        return {}
            ...
            query_builder = query_builder.filter(
                and_(
                    ...
                    or_(
                        ...
                        VehicleBatteryFitment.year_end == None
                    )
                )
            )
    ...
    # --- NEW: AI-Powered Disambiguation ---
    # We found multiple candidates. Use the AI decision-maker to find the best one.
    options_map = {}
    ...
    candidate_strings = list(options_map.keys())
    logger.info(f"Multiple candidates found ({len(candidate_strings)}) for '{user_query}'. Using AI to disambiguate.")

    best_match_key = ai_service.decide_best_vehicle_match(
        user_query=user_query,
        db_candidates=candidate_strings
    )

    if best_match_key and best_match_key in options_map:
        logger.info(f"AI decision-maker selected '{best_match_key}' as the best match.")
        best_fitment = options_map[best_match_key]
        return _format_success_response(db_session, best_fitment)
    else:
        # AI couldn't decide or returned an invalid option. This is a clean failure.
        logger.warning(
            f"AI could not definitively select a match for '{user_query}' from candidates. "
            f"AI response: '{best_match_key}'. Failing search."
        )
        return {"status": "not_found", "message": "No pudimos encontrar la información para tu vehículo."}
...
def update_battery_product_prices(
    ...
    if new_price_discount_fx is not None:
        ...
        price_fx_quantized = new_price_discount_fx.quantize(Decimal('0.empezar de nuevo'), rounding=ROUND_HALF_UP)
